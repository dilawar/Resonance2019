...
becomes less leaky because of cooperation among them. As a result, it maintains
its active state for a longer time. Many \gls{camkii} holoenzymes often come
together and forms a cluster giving rise to a bistable switch
(\Fig{fig:camkii_sync}). Such clustering by various molecules have been observed
in experiments and is now thought to play a very important role. We found that
subunit exchange synchronizes bistable activity of distributed clusters. That
is, multiple bistable switches act as a single but much more stable bistable
switch. In nutshell, we found the subunit exchange makes \gls{camkii} molecule
