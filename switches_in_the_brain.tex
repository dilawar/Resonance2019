...
\boxTexteven{Bistability in reaction network} {
    ...
    property. Life which is remarkably stable is made up of inherently noisy
    molecules. Bistable chemical reaction network are ubiquitous in
    biology where high stability arise out of inherently noisy components.

    A bistable system -- as its name implies -- has two stable states. From the
    point of view of an experimentalist, if you obverse a bistable system for very
    long time, you would almost always find it in either of its two stable
    states. Just like an electrical switch which you would almost always find in
    either \texttt{ON} or \texttt{OFF} state.

    \vspace{3mm} \includegraphics[width=\linewidth]{./stability_noise.pdf} In
    the figure above, \textbf{A} shows an example of deterministic bistable
    system; its stable states are labelled A and B. Any particle released in
    this system will always end up in either A or B. In \textbf{B}, we show
    state transition diagram of a stochastic bistable system.  The transition
    arrow depicts the probability of transition from one state to another. In
    \textbf{C}, we simulate the system described in \textbf{B} with probability
    of transition from A to B, and from B to A set to be 0.01.
    ...
    make a decision e.g. during cell division when a cell needs to decide
    whether to go to next state or not, and in cellular differentiation during
    development when a fate has to be chosen a cell.
    ...
    prevent explosions\cite{wilhelm}. The third one is usually met in
    biological system since reacting molecules and energy available for
    reactions are always in finite supply and that itself prevent explosions.
}

% CaMKII box.
\boxTextodd{

\gls{camkii} at synapse: A brief overview of its properties and function} {
...
\begin{tikzpicture}[baseline, scale=1 , every node/.style={} ]
    \node[label=north west:\bf{A}] (a) {
            \includegraphics[width=1.5cm]{./camkii_pdb.png}
        };
    \node[right=of a.north east, anchor=north west, label=north west:\bf{B}] (b) {
            \includegraphics[width=65mm]{./camkii_subunit_exchage.pdf}
        };
    \node[below=4mm of a.south west, anchor=north west, label=north west:\bf{C}] (c) {
        \includegraphics[width=50mm]{./camkii_pp1_switch_level1_detail.pdf}
        };
    \node[right=of c.north east, anchor=north west, label=north west:\bf{D}] (d) {
            \includegraphics[width=65mm]{./integrator.pdf}
    };
