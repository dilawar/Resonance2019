...
\begin{figure}[ht!]
    \centering
    \caption{Bistable and multi-stable synapse.
        \textbf{(A)} A bistable switch controlling the synaptic strength.
    Synapse size changes in \textit{all-or-none} fashion. \textbf{(B)} A graded
synapse (blue) which changes its size in step-wise manner. A multi-stable mechanism can explain it. Note that multi-stable }
