...
% Bistable box
\boxTexteven{Bistability in reaction networks} {
    ...
    another. In \textbf{(C)}, we show a one trajectory generated by the system
    described in \textbf{B} with probability of transition from A to B, and from B to
    A set to be 0.01. 
}

% CaMKII box.
\boxTextodd{\gls{camkii} at synapse: A brief overview of its properties and
    function} { Among more than 2000 proteins found in brain, \gls{camkii}
    constitutes roughly 2\% of all. Furthermore, it is enriched in Hippocampus
    -- a brain structure essential for memory formation. Indeed, \gls{camkii} is
    known to play essential role in memory formation. In experiments involving
    mice, deactivating \gls{camkii} in any way has always resulted in impairment
    of memory formation and learning. 
