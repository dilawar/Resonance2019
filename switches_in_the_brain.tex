...
what happens to the old memories?
...
Lets assume that $X$ is converted to its active form $X^*$  by adding a
\rightHighlight{
    \raggedright
...
time of memory formation, we had 100 molecules of $X^*$ in synapse. And let's
also assume that on average, every day one new formed molecule (i.e. $X$)
...
\caption{A hypothetical network which can solve the problem of chemical noise and
    ...
    noise do not cause system to activate itself. $X^*$ also partially activate
...
We want a switch like behaviour i.e. if it is \textit{OFF} or \text{ON} it tends
to stay \textit{OFF} or \texttt{ON}. If a few $X$ss are turned into $X^*$ by
background noise, we want them to be quickly turned back into $X$ by
phosphatase. And if during memory formation, a significant portion of $X$ has
been turned into $X^*$ then we want that any $X^*$ deactivated into $X$ is
...
surprising if we find bistable switches in syanpses as well. Indeed, various
studies have shown that \gls{camkii} \emph{may} form a bistable switch in
synapse.
...
form a cluster, then they give rise to a bistable switch
(\Fig{fig:camkii_sync}). A synapse is likely to have many such clusters. We
found subunit exchange synchronizes these distributed switch. That is multiple
bistable switches act as a single but much more stable bistable switch. To
summarise, we found the subunit-exchange makes \gls{camkii} molecule better at
